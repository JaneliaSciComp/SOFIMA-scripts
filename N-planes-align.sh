...
# e.g. ./N-planes-align.sh "data-aphid-N-planes" 10770 10780 100 20 4096
...
bsub_stdout=`bsub ${bsub_flags[@]} -oo $logfile \
    ...
    python -u ./N-planes-flow.py $data_loader $min_z $max_z $patch_size $stride $batch_size`
...
bsub_stdout=`bsub ${bsub_flags[@]} -oo $logfile -w $dependency \
    conda run -n multi-sem --no-capture-output \
    python -u ./N-planes-mesh.py $data_loader $min_z $max_z $patch_size $stride $batch_size`
jobid=`expr match "$bsub_stdout" "$jobid_regex"`
dependency=done\($jobid\)
...
bsub_stdout=`bsub ${bsub_flags[@]} -oo $logfile -w $dependency \
    conda run -n multi-sem --no-capture-output \
   python -u ./N-planes-invmap.py $data_loader $min_z $max_z $patch_size $stride`
jobid=`expr match "$bsub_stdout" "$jobid_regex"`
dependency=done\($jobid\)
...
bsub_stdout=`bsub ${bsub_flags[@]} -oo $logfile -w $dependency \
    conda run -n multi-sem --no-capture-output \
    python -u ./N-planes-warp.py $data_loader $min_z $max_z $patch_size $stride`
jobid=`expr match "$bsub_stdout" "$jobid_regex"`
dependency=done\($jobid\)
