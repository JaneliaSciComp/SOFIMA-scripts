...
# usage: ./1-plane-stitch.sh <level> <patch_size> <stride> <min_z> <max_z> <outpath>
# e.g. ./1-plane-stitch.sh 0 50 5 10770 10780 /nrs/cellmap/arthurb/stitch.patch50.stride5
...
for slice in "${uniq_slices[@]}" ; do
    ...
    bsub ${bsub_flags[@]} -oo $logfile \
            ...
            python -u ./1-plane-stitch.py "data-aphid-1-plane" $slice $level $patch_size $stride $outpath
done
